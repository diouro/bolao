  ...
  return (
    <div className="space-y-6">
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <StatCard label="Your accuracy" value={`${current?.accuracy ?? 0}%`} />
        <StatCard label="Exact hits" value={current?.exactHits ?? 0} />
        <StatCard label="Best streak" value={current?.bestStreak ?? 0} />
        <StatCard label="Bold picks" value={current?.boldPicks ?? 0} />
      </div>

      <div className="grid gap-4 lg:grid-cols-3">
        <Spotlight title="Most exact scores" user={summary.mostExact} metric="exactHits" />
        <Spotlight title="Hottest streak" user={summary.hottestStreak} metric="bestStreak" />
        <Spotlight title="Boldest picks" user={summary.boldest} metric="boldPicks" />
      </div>
  ...
  return (
    <Card className="bg-zinc-950 text-white">
      ...
      <div className="mt-3 text-2xl font-black">
        {user ? user.profile.display_name ?? user.profile.email : "No one yet"}
      </div>
      <div className="mt-2 text-sm text-zinc-300">
        {user ? `${user[metric]} hits` : "Stats appear after results are entered."}
      </div>
    </Card>
  );
}
